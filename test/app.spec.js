...
describe("express", function () {
  ...
  it("should calculate total change correctly", () =>
    nightmare
      ...
      .evaluate(
        () => document.querySelector("div.alert.alert-success").innerText
      )
      .end()
  ...
  it("should calculate individual change correctly", () =>
    nightmare
      ...
      .wait("div.alert.alert-success")
      .evaluate(() =>
        Array.from(document.querySelectorAll(".change")).map((e) => e.innerText)
      )
      .then((results) => {
        const expected = {
          twenties: "0",
        ...
        const promises = Object.keys(expected).map((key, i) =>
          expect(results[i]).to.equal(
            expected[key],
            `Expected ${expected[key]} ${key}, saw ${results[i]} instead.`
          )
        );
